...
function buildContextForAI() {
  ...
  if (data.personal) {
    ...
    if (data.personal.location) context += `Location: ${data.personal.location}\n`;
    if (data.personal.languages) context += `Languages: ${data.personal.languages}\n`;
    if (data.personal.currentPosition) context += `Current: ${data.personal.currentPosition}\n`;
    if (data.personal.currentResearchPosition) context += `Research: ${data.personal.currentResearchPosition}\n`;
  }
  
  // Full CV details
  if (data.fullCV) {
    if (data.fullCV.education) {
      context += `\n=== EDUCATION ===\n`;
      data.fullCV.education.forEach(e => context += `- ${e}\n`);
    }
    if (data.fullCV.experience) {
      context += `\n=== EXPERIENCE ===\n`;
      data.fullCV.experience.forEach(e => context += `- ${e}\n`);
    }
    if (data.fullCV.publications) {
      context += `\n=== PUBLICATIONS ===\n`;
      context += `Summary: ${data.fullCV.publications.summary}\n`;
      if (data.fullCV.publications.highlights) {
        context += `Highlights:\n`;
        data.fullCV.publications.highlights.forEach(p => context += `- ${p}\n`);
      }
    }
    if (data.fullCV.skills) {
      context += `\n=== SKILLS ===\n`;
      if (data.fullCV.skills.technical) context += `Technical: ${data.fullCV.skills.technical}\n`;
      if (data.fullCV.skills.nlp) context += `NLP: ${data.fullCV.skills.nlp}\n`;
      if (data.fullCV.skills.research) context += `Research: ${data.fullCV.skills.research}\n`;
    }
    if (data.fullCV.achievements) {
      context += `\n=== ACHIEVEMENTS ===\n`;
      data.fullCV.achievements.forEach(a => context += `- ${a}\n`);
    }
    if (data.fullCV.projects) {
      context += `\n=== PROJECTS ===\n`;
      data.fullCV.projects.forEach(p => context += `- ${p}\n`);
    }
    if (data.fullCV.researchInterests) {
      context += `\n=== RESEARCH INTERESTS ===\n${data.fullCV.researchInterests}\n`;
    }
  }
